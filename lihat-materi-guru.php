					...
					<table cellpadding="0" cellspacing="0" border="0" id="table" class="tinytable">
						<thead>
							<tr>
								...
								<th><h3 class="uk-text-center">Guru</h3></th>
						...
						$nip = "SELECT * from guru where nip ='$username'";
						$lihat=mysql_query($nip);
						$cek_guru=mysql_fetch_array($lihat);
						$user=$cek_guru['nip'];
						  $query="SELECT * FROM materi WHERE nip='$user'";
						  $hasil=mysql_query($query);
						  $no=0;
						  while ($data=mysql_fetch_array($hasil)) { $no++; 
						  	$no = $data['kd_materi'];
            				$guru = mysql_fetch_array(mysql_query("SELECT * from guru where nip = ".$data['nip'].""));
            				$nm_guru = $guru['nm_guru'];
							$kelas=mysql_fetch_array(mysql_query("SELECT * from kelas INNER JOIN materi on materi.kd_kelas = kelas.kd_kelas where kd_materi = '$kd_materi'"));
							$kd_kelas=$kelas['kd_kelas'];
          			 	  	$mapel=mysql_fetch_array(mysql_query("SELECT * FROM mapel INNER JOIN materi ON materi.kd_mapel = mapel.kd_mapel where kd_materi = '$kd_materi'"));
        				    ...
        				    ?>
							  ...
							  <tr>
								...
								<td><div class="uk-text-center"><?php echo $judul_materi?></div></td>
								<td><div class="uk-text-center"><?php echo $nama_file?></div></td>
								<td><div class="uk-text-center"><?php echo $size?></div></td>
								<td><div class="uk-text-center"><?php echo $waktu?></div></td>
						          <?php if (isset($_SESSION['pengguna'])) { ?>
								<td><div class="uk-text-center">
								  <a href="materi-guru.update?id=<?php echo $data['kd_materi']?>" title="Sunting" data-uk-tooltip="{pos:'top-left'}" class="uk-button uk-button-small"><i class="uk-icon-pencil"></i></a>
								  <a href="materi-guru.hapus?id=<?php echo $data['kd_materi']?>" onclick="return confirm('Apakah anda yakin akan menghapus data Materi: <?php echo $row[1] ?> ini?')" title="Hapus" data-uk-tooltip="{pos:'top-left'}" class="uk-button uk-button-small uk-button-danger"><i class="uk-icon-remove"></i></a>
								</div>
								</td>
								<?php } ?>						
							  </tr>
